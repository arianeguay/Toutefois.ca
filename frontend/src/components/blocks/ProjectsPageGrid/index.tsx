...
import { GridContainer, GridItem, ProjectImage, ProjectTitle, ProjectExcerpt, PaginationContainer, PaginationButton } from './styles';
  ...
  useEffect(() => {
    const getProjects = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost/wp-json/toutefois/v1/projects-grid?page=${page}&per_page=9`);
      ...
      <GridContainer>
        {projects.map((project) => (
          <GridItem key={project.id}>
            ...
            <ProjectExcerpt dangerouslySetInnerHTML={{ __html: project.excerpt }} />
          </GridItem>
        ))}
      </GridContainer>
      <PaginationContainer>
